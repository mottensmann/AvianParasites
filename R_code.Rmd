...
```{r, eval=FALSE}
## read taxon names
taxon <- readxl::read_xlsx("data/dataset_mo.xlsx")
taxon <- taxon[["taxon.iucn"]] %>% as.character()
dim(taxon) # 1576 species in total 
...
df <- readxl::read_xlsx("data/dataset_mo.xlsx") %>% 
...
# df <- data.frame(taxon.iucn = dataset[["taxon.iucn"]],
#                  taxon.hbw = dataset[["taxon.hbw"]],
#                  taxon.malavi = dataset[["taxon.malavi"]],
#                  taxon.english = common.names[["taxon.english"]]) %>% 
#   dplyr::mutate_all(., as.character)

# test <-
  # match_synonyms(df = df,
                 # BirdFuncDat = BirdFuncDat)
# write.csv(data.frame(taxon = dataset$taxon.iucn, match = test), "data/temp.csv")
```
